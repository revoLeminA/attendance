...
Route::group(['middleware' => ['verified']], function () {
    ...
    Route::group(['middleware' => ['auth:web']] ,function () {
        ...
        Route::post('/attendance', [AttendanceController::class, 'store'])->name('user.attendance.store');      // 勤怠登録処理（一般ユーザ）
        Route::patch('/attendance', [AttendanceController::class, 'update'])->name('user.attendance.update');   // 勤怠更新処理（一般ユーザ）
    ...
    Route::group(['middleware' => ['auth:admin,web']] ,function () {
        ...
        Route::patch('/attendance/{id}', [AttendanceDetailController::class, 'update'])->name('auth.attendance.update');    // 勤怠修正処理（一般ユーザ、管理者）
...
Route::group(['prefix' => '/admin'], function () {
    ...
    Route::post('/login', [AdminAuthenticateController::class, 'store']);                           // ログイン処理（管理者）
    Route::post('/logout', [AdminAuthenticateController::class, 'destroy']);                        // ログアウト処理（管理者）
});
...
Route::group(['middleware' => ['auth:admin']], function () {
    ...
    Route::post('/admin/attendance/staff/{id}', [AdminStaffAttendanceController::class, 'export'])->name('admin.staff.attendance.csv');                         // CSV出力機能（管理者）
    ...
    Route::patch('/stamp_correction_request/approve/{attendance_correct_request}', [AdminRequestController::class, 'update'])->name('admin.request.update');    // 修正申請承認処理（管理者）
});
